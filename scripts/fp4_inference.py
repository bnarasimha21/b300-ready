...
B300 Blackwell introduces FP4 (NVFP4) for 2x throughput vs FP8.
This example demonstrates FP4 inference using TensorRT-LLM.
...
# Check for TensorRT-LLM (required for FP4)
try:
    import tensorrt_llm
...
# Alternative: Transformer Engine FP4 (experimental)
try:
...
def fp4_with_tensorrt_llm():
    """
    FP4 inference using TensorRT-LLM (recommended for production)
    
    Build FP4 engine:
        python convert_checkpoint.py --model_dir ./llama-7b \
            --output_dir ./llama-7b-fp4 \
            --dtype float16 \
            --use_fp4_weights \
            --fp4_weight_quantization_method awq
    """
    from tensorrt_llm import LLM, SamplingParams
    
    # Load FP4-quantized model
    llm = LLM(
        model="./llama-7b-fp4",  # Pre-converted FP4 checkpoint
        tensor_parallel_size=8,
        dtype="float16",
        quantization="fp4_awq",  # NVFP4 quantization
    )
...
def fp4_quantization_example():
    """
    Quantize a model to FP4 using TensorRT-LLM
    """
    ...
    # Quantize to FP4 with AWQ calibration
    quantized_model = quantize_model(
        model,
        quant_mode=QuantMode.use_weight_only(use_fp4=True),
        calib_dataset="cnn_dailymail",  # Calibration dataset
        calib_batches=512,
    )
    
    # Save
    quantized_model.save("./llama-7b-fp4")
    print("Saved FP4 model to ./llama-7b-fp4")


def compare_precisions():
    """
    Compare FP4 vs FP8 vs FP16 throughput on B300
    """
...
def main():
    ...
    if args.mode == "compare":
        ...
        if not HAS_TRTLLM:
            print("TensorRT-LLM required for FP4 inference benchmark")
            return
        fp4_with_tensorrt_llm()
    elif args.mode == "quantize":
        if not HAS_TRTLLM:
            print("TensorRT-LLM required for FP4 quantization")
            return
        fp4_quantization_example()
